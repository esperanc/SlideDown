...
class ScrollyRenderer {
  ...
  constructor(containerId, options = {}) {
    ...
    this.markdownConverter = options.markdownConverter || ((text) => {
      ...
      processedText = processedText.replace(/\$\$([\s\S]+?)\$\$/g, (match, content) => {
        ...
        mathBlocks.push({ content: content.trim(), displayMode: true });
        return placeholder;
      });
      ...
      processedText = processedText.replace(/\$([^\$\n]+?)\$/g, (match, content) => {
        ...
        mathBlocks.push({ content: content.trim(), displayMode: false });
        return placeholder;
      });
      ...
      // Restore math expressions by rendering with KaTeX
      mathBlocks.forEach((mathObj, index) => {
        let renderedMath;
        try {
          if (typeof katex !== 'undefined') {
            ...
            // KaTeX not loaded yet, preserve the delimiters for auto-render
            renderedMath = mathObj.displayMode 
              ? `$$${mathObj.content}$$` 
              : `$${mathObj.content}$`;
          }
        } catch (e) {
          console.error('KaTeX rendering error:', e);
          renderedMath = `<span class="katex-error">${mathObj.content}</span>`;
        }
        
        if (mathObj.displayMode) {
          const placeholder = `MATH_BLOCK_${index}`;
          // Check if placeholder is wrapped in <p> tags and remove them
          const pWrappedPattern = new RegExp(`<p>${placeholder}</p>`, 'g');
          if (pWrappedPattern.test(html)) {
            html = html.replace(pWrappedPattern, `<div class="katex-block">${renderedMath}</div>`);
          } else {
            html = html.replace(placeholder, `<div class="katex-block">${renderedMath}</div>`);
          }
        } else {
          html = html.replace(`MATH_INLINE_${index}`, renderedMath);
        }
      });
    ...
    this.componentRegistry = {
      ...
      image: (params) => {
        ...
        if (params.height) {
            // height=XX% is intended to mean XX% of the slide's visible area.
            // However, percentage heights on deeply nested flex children resolve
            // against the nearest ancestor with a definite height — which can be
            // the image's intrinsic size rather than the slide. To guarantee correct
            // resolution at any nesting depth, convert XX% → a calc() in vh units
            // that mirrors the slide's own height formula (100vh - 40px - 6rem padding).
            // Non-percentage values (px, vh, em…) are passed through unchanged.
            const percentMatch = params.height.match(/^(\d+(?:\.\d+)?)%$/);
            const resolvedHeight = percentMatch
                ? `calc(${percentMatch[1]} * (100vh - 40px - 6rem) / 100)`
                : params.height;
            wrapper.style.maxHeight = resolvedHeight;
  ...
  // Um valor de tamanho é "relativo" quando é uma percentagem (ex: "40%").
  _isPercent(value) {
    return typeof value === 'string' && /^\d+(?:\.\d+)?%$/.test(value.trim());
  }

  // --- SIZING DE FILHOS (width / height) ---
  // Como 'width' e 'height' se resolvem depende do eixo do container pai:
  //  - Numa 'row' (eixo horizontal) 'width' é o tamanho ao longo do eixo
  //    principal, portanto vira flex-basis; 'height' é o eixo cruzado.
  //  - Numa 'col' (ou no próprio slide) 'width' é o eixo cruzado e aplica-se
  //    diretamente.
  // Alturas em percentagem NÃO são resolvidas aqui: 100% significa "o espaço
  // vertical disponível na janela", medido em tempo de layout por
  // ScrollyNavigation._applyBlockHeights().
  _applyChildSizing(containerEl) {
    const isRow = containerEl.classList.contains('layout-row');
    const children = Array.from(containerEl.children);

    children.forEach(child => {
      const width = child.dataset.sdWidth;
      const height = child.dataset.sdHeight;
      ...
      // Alturas absolutas (px, vh, em...) aplicam-se de imediato. Só se congela
      // o flex quando a altura é o eixo principal: numa 'row' é o eixo cruzado
      // e mexer no flex estragaria a largura da coluna.
      if (height && !this._isPercent(height)) {
    ...
    // Larguras em percentagem competem com o 'gap: 2%' da .layout-row: duas
    // colunas de 50% somariam 102% e a segunda passaria para a linha seguinte.
    // Só encolhemos quando há de facto excesso — width=30% continua a ser 30%.
    if (!isRow) return;

    const sized = children.filter(c => this._isPercent(c.dataset.sdWidth));
    if (sized.length < 2) return;

    const gapPercent = 2; // igual ao 'gap' de .layout-row no CSS
    const totalGap = (sized.length - 1) * gapPercent;
    const total = sized.reduce((sum, c) => sum + parseFloat(c.dataset.sdWidth), 0);

    if (total === 0 || total + totalGap <= 100) return;

    const scale = (100 - totalGap) / total;
    sized.forEach(c => {
      c.style.flexBasis = `${parseFloat(c.dataset.sdWidth) * scale}%`;
    });
  }
    ...
    if (node.type === 'slide') {
      ...
      // Se for o primeiro (mais externo), aplica os parâmetros e estilos
          ...
          if (index === 0) {
              ...
              // width/height ficam registados no elemento: a forma como se
              // resolvem depende do eixo do pai (ver _applyChildSizing) e, no
              // caso das alturas em percentagem, da geometria do slide
              // (ver ScrollyNavigation._applyBlockHeights).
              if (node.params && node.params.width) {
                  currentEl.dataset.sdWidth = node.params.width;
              }
              if (node.params && node.params.height) {
                  currentEl.dataset.sdHeight = node.params.height;
              }
      ...
      this._applyChildSizing(innerEl);
