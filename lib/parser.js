class ScrollyParser {
  constructor() {
    ...
    this.allowedBlocks = ['row', 'col', 'column', 'center', 'section', 'reveal', 'reveal*'];
    this.allowedComponents = ['image', 'img', 'youtube', 'iframe'];
  }
  ...
  _parseSlide(text, index) {
    ...
    root.children.forEach(child => {
      ...
      const isImage = child.type === 'component' && (child.name === 'img' || child.name === 'image');
  ...
  _parseParams(argString) {
    ...
    // 3. Check for standalone ratio parameter (percentage without key=value format)
    // This allows ":::col 70% reveal" in addition to ":::col ratio=70% reveal"
    // Só percentagens soltas contam: um '%' que faz parte de um par key=value
    // (ex: "height=100%") não pode ser confundido com um ratio.
    if (!params.ratio) {
        const leftovers = trimmed.replace(regex, ' ');
        const percentMatch = leftovers.match(/(\d+(?:\.\d+)?)%/);
        if (percentMatch) {
            params.ratio = percentMatch[1] + '%';
        }
    }
