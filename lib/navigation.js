class ScrollyNavigation {
  ...
  _setupScrollSteps(specificSlide = null) {
    ...
    slidesToProcess.forEach(slide => {
      ...
      // Instead, compare each column's bounding rect against the slide's clipping edge.
      const colsInSlide = targetForQuery.querySelectorAll('.layout-col');

      // WRITE phase: reset inline constraints from any previous run.
      ...
      const colMeasurements = Array.from(colsInSlide).map(col => ({
        col,
      ...
      colMeasurements.forEach(({ col, colBox }) => {
        const overflowAmount = colBox.bottom - clipBox.bottom;
      ...
      const referenceH = window.innerHeight - 40;
