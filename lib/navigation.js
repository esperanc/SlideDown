class ScrollyNavigation {
  ...
  _setupScrollSteps() {
    this.slides.forEach(slide => {
      ...
      // --- OVERFLOW DETECTION ---
      // Look for columns and content that have overflowing content
      // Rows are layout containers (side-by-side), columns can scroll independently
      const layoutContainers = slide.querySelectorAll('.layout-col, .md-content');
      ...
      layoutContainers.forEach(el => {
        ...
        if (el.closest('.has-overflow')) return;
        
        // Calculate overflow based on content vs CONSTRAINED height
        // Detection runs before CSS is applied, so use the target constraint
        const scrollH = el.scrollHeight;
        const targetHeight = Math.min(el.clientHeight, constrainedHeight);
        ...
        if (expectedOverflow > 10) { // 10px tolerance
          ...
          console.log(`[Overflow] Detected: ${steps} steps (scrollH=${scrollH}, targetH=${targetHeight}, overflow=${expectedOverflow})`, el);
        }
      });
      
      // Add the maximum overflow steps (not sum of all)
